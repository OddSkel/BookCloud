...
use crate::grpc::contracts::{
    author_analytics::{
        author_analytics_grpc_server::AuthorAnalyticsGrpc, AuthorConsistencyItem,
        AuthorEvolutionPoint, AuthorGrowthItem, AuthorPerformanceRequest,
        AuthorPerformanceResponse, AuthorsConsistencyRequest, AuthorsConsistencyResponse,
        AuthorsGrowthRequest, AuthorsGrowthResponse, RankAuthorRequest, RankAuthorResponse,
        RankedAuthor,
    },
...
impl AuthorAnalyticsService {
    pub fn new(db: AuthorAnalyticsDb) -> Self {
        Self { db }
    }
}
...
impl AuthorAnalyticsGrpc for AuthorAnalyticsService {
    ...
    async fn rank_authors(
        &self,
        request: Request<RankAuthorRequest>,
    ...
    async fn author_performance(
        ...
        let evolution: Vec<AuthorEvolutionPoint> = rows
            .iter()
            .map(|r| AuthorEvolutionPoint {
                year: r.pub_year,
    ...
    async fn authors_consistency(
        &self,
        request: Request<AuthorsConsistencyRequest>,
    ) -> Result<Response<AuthorsConsistencyResponse>, Status> {
        let p = request.into_inner();
        let rows = self
            .db
            .author_rating_per_book(p.author_name.as_deref(), p.author_id)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
        ...
        for r in &rows {
            per_author
                .entry((r.author_id, r.author_name.clone()))
                .or_default()
                .push(r.star_rating);
        }
        ...
        let mut results: Vec<AuthorConsistencyItem> = per_author
            ...
            .map(|((id, name), ratings)| {
                ...
                let var =
                    ratings.iter().map(|r| (r - avg).powi(2)).sum::<f64>() / ratings.len() as f64;
        ...
        results
            .iter_mut()
            .enumerate()
            .for_each(|(i, r)| r.rank = (i + 1) as i32);

        Ok(Response::new(AuthorsConsistencyResponse {
            authors: results,
        }))
    }
    ...
    async fn authors_growth(
        &self,
        request: Request<AuthorsGrowthRequest>,
    ) -> Result<Response<AuthorsGrowthResponse>, Status> {
        let p = request.into_inner();
        let rows = self
            .db
            .author_books_chronological(p.author_name.as_deref(), p.author_id)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
        ...
        let mut results: Vec<AuthorGrowthItem> = per_author
            ...
            .map(|((id, name), books)| {
                ...
                let latest =
                    books[mid..].iter().map(|(_, r)| r).sum::<f64>() / (books.len() - mid) as f64;
                AuthorGrowthItem {
                    author_id: id.to_string(),
                    author_name: name,
...
mod tests {
    ...
    fn test_rank_authors_by_avg_rating_orders_correctly() {
        ...
        assert_eq!(
            result[0].author_name, "Bob Books",
            "Bob (4.8) must rank first by avg rating"
        );
        assert_eq!(
            result[1].author_name, "Alice Author",
    ...
    fn test_consistency_std_dev_computed_correctly() {
        ...
        let alice = result
            .iter()
            .find(|r| r.author_name == "Alice Author")
            .expect("Alice must appear");
        assert!(
            (alice.std_deviation - 0.75).abs() < 1e-9,
            "std_deviation should be 0.75, got {}",
            alice.std_deviation
        );
        assert!(
            (alice.average_rating - 3.75).abs() < 1e-9,
            "average_rating should be 3.75, got {}",
            alice.average_rating
        );
    ...
    fn test_consistency_single_book_author_has_zero_std_dev() {
        ...
        let rows = vec![author_book_row(2, "Bob Books", 1003, 2022, 4.8, 200)];
        let result = compute_consistency(rows);
        let bob = result
            .iter()
            .find(|r| r.author_name == "Bob Books")
            .expect("Bob must appear");
        assert!(
            bob.std_deviation.abs() < 1e-9,
            "Single-book author must have std_deviation=0.0"
        );
        assert!(
            (bob.consistency_score - 1.0).abs() < 1e-9,
    ...
    fn test_growth_excludes_single_book_authors() {
        ...
        assert!(
            !result.iter().any(|r| r.author_name == "Bob Books"),
    ...
    fn test_growth_score_is_latest_minus_first() {
        ...
        let alice = result
            ...
            .expect("Alice must appear");
        assert!(
            (alice.growth_score - (-1.5)).abs() < 1e-9,
            "growth_score should be -1.5, got {}",
            alice.growth_score
        );
    ...
    fn test_growth_positive_score_when_improving() {
        ...
        let carol = result
            .iter()
            .find(|r| r.author_name == "Carol Writes")
            .expect("Carol must appear");
        assert!(
            carol.growth_score > 0.0,
    ...
    fn test_growth_ranked_descending_by_growth_score() {
        ...
        let result = compute_growth(rows);
        assert_eq!(
            result[0].author_name, "Carol Writes",
    ...
    fn test_pearson_correlation_perfect_positive() {
        ...
        assert!(
            (r - 1.0).abs() < 1e-9,
            "Perfect positive correlation must return ~1.0, got {}",
            r
        );
    }
    ...
    fn test_pearson_correlation_perfect_negative() {
        ...
        assert!(
            (r - (-1.0)).abs() < 1e-9,
            "Perfect negative correlation must return ~-1.0, got {}",
            r
        );
    }
