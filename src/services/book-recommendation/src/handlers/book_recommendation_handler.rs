...
use crate::grpc::contracts::genre_service::{GetBooksByGenreRequest, genre_analysis_grpc_client::GenreAnalysisGrpcClient};
use crate::grpc::contracts::rating_catalog::{GetBooksByRatingRequest, GetBooksByPopularityRequest, rating_catalog_grpc_client::RatingCatalogGrpcClient};
...
pub async fn book_recommendation(
    ...
    if let Some(genre) = params.genre.as_deref().filter(|s| !s.trim().is_empty()) {
        let mut genre_client = GenreAnalysisGrpcClient::connect(genre_analysis_grpc_url.to_string())
            .await
        ...
        book_isbns = genre_response.items.iter()
            .map(|i| i.isbn)
            .collect();
    }
    ...
    if params.rating.is_some() || params.popularity.is_some() {
        let mut rating_client = RatingCatalogGrpcClient::connect(rating_catalog_grpc_url.to_string())
            .await
            .map_err(|e: TonicError| e.to_string())?;
    ...
    if book_isbns.is_empty() {
        let rows: Vec<(i64,)> = sqlx::query_as("SELECT isbn FROM books")
            .fetch_all(pool)
            .await
